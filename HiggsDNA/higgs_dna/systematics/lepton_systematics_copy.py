...
def electron_scale_smear_run3(events, year, is_data):
    ...
    logger.debug(
        "[Lepton Systematics] Electron pt Scale Up (dEscaleUp) (first %d): %s",
        _nprint,
        awkward.to_numpy(awkward.flatten(events["Electron", "dEscaleUp"]))[:_nprint],
    )
    logger.debug(
        "[Lepton Systematics] Electron pt Scale Down (dEscaleDown) (first %d): %s",
        _nprint,
    ...
    logger.debug(
        ...
        _nprint,
        awkward.to_numpy(awkward.flatten(events["Electron", "dEsigmaUp"]))[:_nprint],
    )
    logger.debug(
