...
DEFAULT_OPTIONS = {
    "photons" : {
        ...
        "e_veto" : 0.5,
        # ------------------------------------------------------------
        # AN-20-142 §4.2 photon ID (merged a -> gamma gamma, mA in [0.1, 1.2] GeV)
        # Cuts are R9-tiered; barrel only.
        # ------------------------------------------------------------
        "AN2020_r9_min": 0.5,
        "AN2020_r9_highR9": 0.85,
        "AN2020_hoe_max": 0.04596,
        "AN2020_sieie_max_lowR9": 0.015,        # only for 0.5 < R9 <= 0.85
        "AN2020_ecalPFClusIso_max_lowR9": 4.0,  # GeV, after PU corr
        "AN2020_trkIso_max_lowR9": 6.0,         # GeV, trkSumPtHollowConeDR03
        "AN2020_trkIso_rel_max": 0.07,          # I_tk / pT,Gamma
        "AN2020_mvaID_min": -0.96,
        "AN2020_abs_eta_max": 1.4,              # barrel only
    },
    "zgammas" : {
...
class ZaTaggerRun3(Tagger):
    ...
    def produce_and_select_zgammas(self, events, rho, options):
        ...
        # Merged category (AN-20-142, low-mA a -> single Gamma): exactly 1 selected photon.
        # `photons` here is already lepton-cleaned + AN/standard photon-ID selected.
        has_1merged_cand = ak.fill_none(ak.num(photons) == 1, False)
        awkward_utils.add_field(events, "n_photons_sel", ak.fill_none(ak.num(photons), 0), overwrite=True)
        awkward_utils.add_field(events, "has_1merged_cand", has_1merged_cand, overwrite=True)
        ...
        if self.options.get("zgammas", {}).get("study_photon_id_scenarios", False):
            scenario_keys = [
                ...
                "phid_official_loose",
                # AN-20-142 §4.2 (low-mA merged a->gg)
                "phid_AN2020",
            ]
        ...
        # ---------------------------------------------------------------
        # Merged category cutflow (AN-20-142 low-mA path)
        #   - exactly 1 photon passing the AN2020 photon ID
        #   - lepton/Z and Higgs window reused from the standard path
        # ---------------------------------------------------------------
        if "pass_phid_AN2020" in events.Photon.fields:
            n_an_pho = ak.sum(events.Photon["pass_phid_AN2020"], axis=1)
            has_1merged_AN = ak.fill_none(n_an_pho == 1, False)
            ...
            for weighted in (False, True):
                ...
                if not weighted:
                    awkward_utils.add_field(events, "pass_allcuts_merged_AN2020", ak.fill_none(final_cut, False), overwrite=True)

        # ---------------------------------------------------------------
        # MLPhoton (AN-20-142 end-to-end m_Gamma regressor) wiring
        #   - leading MLPhoton picked by diphotonScore (best diphoton-like candidate)
        #   - per-event scalars stored for downstream ROI residual computation
        #   - merged_ML cutflow analogous to merged_AN2020
        # ---------------------------------------------------------------
        if "MLPhoton" in events.fields:
            ...
            # diphoton-like candidate; tie-break: nothing -- diphotonScore alone.
            ml_sorted = ml[ak.argsort(ml.diphotonScore, axis=1, ascending=False)]
            ml_lead = ak.firsts(ml_sorted)

            for f in ("pt", "eta", "phi", "mass", "massEnergyRatio",
                      "diphotonScore", "monophotonScore", "hadronScore",
                      "pfIsolation", "r1", "r2", "r3"):
                awkward_utils.add_field(
                    events,
                    f"MLPhoton_lead_{f}",
                    ak.fill_none(getattr(ml_lead, f), DUMMY_VALUE),
                    overwrite=True,
                )
    ...
    def calculate_gen_info(self, zgammas, options):
        ...
        # ------------------------------------------------------------
        # Low-mA merged proxy (AN-20-142 phenomenology, §2.1):
        #   gamma_L = E_a / m_a  -- Lorentz boost of the a in detector frame
        #   m_a (truth)          -- proxy for the missing regressed m_Gamma
        # ------------------------------------------------------------
        a_p4 = ak.zip(
            {"pt": a_cand.pt, "eta": a_cand.eta, "phi": a_cand.phi, "mass": a_cand.mass},
            with_name="Momentum4D",
        )
        a_energy = ak.fill_none(a_p4.energy, DUMMY_VALUE)
        a_mass = ak.fill_none(a_cand.mass, DUMMY_VALUE)
        gamma_L = ak.where(a_mass > 0, a_energy / a_mass, DUMMY_VALUE)
        awkward_utils.add_field(zgammas, "GenALP_mass", a_mass, overwrite=True)
        awkward_utils.add_field(zgammas, "GenALP_gammaL", ak.fill_none(gamma_L, DUMMY_VALUE), overwrite=True)

        # Truth-match the merged single-photon candidate.
        # For low mA, both a -> gamma gamma daughters reconstruct as one Photon;
        # we match the gen a by deltaR to the closest SelectedPhoton in the event.
        try:
            sel_photons = zgammas.SelectedPhoton
            a_eta = a_cand.eta[:, None]
            a_phi = a_cand.phi[:, None]
            deta = sel_photons.eta - a_eta
            dphi = (sel_photons.phi - a_phi + numpy.pi) % (2 * numpy.pi) - numpy.pi
            dr_a_pho = numpy.sqrt(deta * deta + dphi * dphi)  # (ev, n_pho)

            best_idx = ak.argmin(dr_a_pho, axis=1, keepdims=True)
            min_dr = ak.min(dr_a_pho, axis=1)  # (ev,) -- None when n_pho==0
            matched_pho = ak.firsts(sel_photons[best_idx])

            awkward_utils.add_field(zgammas, "MergedGamma_dR_to_genA",
                                    ak.fill_none(min_dr, DUMMY_VALUE), overwrite=True)
            awkward_utils.add_field(zgammas, "MergedGamma_pt",
                                    ak.fill_none(matched_pho.pt, DUMMY_VALUE), overwrite=True)
            awkward_utils.add_field(zgammas, "MergedGamma_eta",
                                    ak.fill_none(matched_pho.eta, DUMMY_VALUE), overwrite=True)
            awkward_utils.add_field(zgammas, "MergedGamma_phi",
                                    ak.fill_none(matched_pho.phi, DUMMY_VALUE), overwrite=True)
            awkward_utils.add_field(zgammas, "MergedGamma_r9",
                                    ak.fill_none(matched_pho.r9, DUMMY_VALUE), overwrite=True)
            awkward_utils.add_field(zgammas, "MergedGamma_mvaID",
                                    ak.fill_none(matched_pho.mvaID, DUMMY_VALUE), overwrite=True)
            awkward_utils.add_field(zgammas, "MergedGamma_hoe_PUcorr",
                                    ak.fill_none(matched_pho.hoe_PUcorr, DUMMY_VALUE), overwrite=True)
            awkward_utils.add_field(zgammas, "MergedGamma_sieie",
                                    ak.fill_none(matched_pho.sieie, DUMMY_VALUE), overwrite=True)
            if "pass_phid_AN2020" in sel_photons.fields:
                awkward_utils.add_field(
                    zgammas,
                    "MergedGamma_pass_phid_AN2020",
                    ak.fill_none(ak.firsts(sel_photons.pass_phid_AN2020[best_idx]), False),
                    overwrite=True,
                )
        except Exception as _e:
            logger.debug(f"[merged truth match] skipped: {_e}")

        # ------------------------------------------------------------
        # Truth-match the leading MLPhoton candidate to gen ã and
        # compute the m_Gamma regression residual (the "ROI" in AN §5.4).
        # ------------------------------------------------------------
        if "MLPhoton" in zgammas.fields:
            try:
                ml = zgammas.MLPhoton
                a_eta = a_cand.eta[:, None]
                a_phi = a_cand.phi[:, None]
                ...
                dr_a_ml = numpy.sqrt(deta * deta + dphi * dphi)

                best_idx = ak.argmin(dr_a_ml, axis=1, keepdims=True)
                min_dr = ak.min(dr_a_ml, axis=1)
                matched_ml = ak.firsts(ml[best_idx])

                ml_mass = ak.fill_none(matched_ml.mass, DUMMY_VALUE)
                gen_ma = ak.fill_none(a_cand.mass, DUMMY_VALUE)
                residual = ak.where(
                    (ml_mass > -100) & (gen_ma > -100),
                    ml_mass - gen_ma,
                    DUMMY_VALUE,
                )

                awkward_utils.add_field(zgammas, "MergedML_dR_to_genA",
                                        ak.fill_none(min_dr, DUMMY_VALUE), overwrite=True)
                awkward_utils.add_field(zgammas, "MergedML_pt",
                                        ak.fill_none(matched_ml.pt, DUMMY_VALUE), overwrite=True)
                awkward_utils.add_field(zgammas, "MergedML_eta",
                                        ak.fill_none(matched_ml.eta, DUMMY_VALUE), overwrite=True)
                awkward_utils.add_field(zgammas, "MergedML_phi",
                                        ak.fill_none(matched_ml.phi, DUMMY_VALUE), overwrite=True)
                awkward_utils.add_field(zgammas, "MergedML_mass", ml_mass, overwrite=True)
                awkward_utils.add_field(zgammas, "MergedML_massEnergyRatio",
                                        ak.fill_none(matched_ml.massEnergyRatio, DUMMY_VALUE), overwrite=True)
                awkward_utils.add_field(zgammas, "MergedML_diphotonScore",
                                        ak.fill_none(matched_ml.diphotonScore, DUMMY_VALUE), overwrite=True)
                awkward_utils.add_field(zgammas, "MergedML_monophotonScore",
                                        ak.fill_none(matched_ml.monophotonScore, DUMMY_VALUE), overwrite=True)
                awkward_utils.add_field(zgammas, "MergedML_hadronScore",
                                        ak.fill_none(matched_ml.hadronScore, DUMMY_VALUE), overwrite=True)
                awkward_utils.add_field(zgammas, "MergedML_pfIsolation",
    ...
    def select_photons(self, photons, options, electrons, rho, year):
        ...
        # ------------------------------------------------------------
        # AN-20-142 §4.2 photon ID (merged low-mA a -> gamma gamma)
        # Barrel only, R9-tiered cuts on sieie / ECAL iso / track iso.
        # Requires Run3 PU-corrected branches; dummy False for Run2.
        # ------------------------------------------------------------
        if int(year) <= 2020:
            phid_AN2020 = ak.zeros_like(photons.pt, dtype=bool)
        else:
            an_r9 = photons.r9 > options["AN2020_r9_min"]
            an_eta = photon_abs_eta < options["AN2020_abs_eta_max"]
            an_hoe = photons.hoe_PUcorr < options["AN2020_hoe_max"]
            # pixel seed veto: NanoAOD has `pixelSeed`; fall back to electronVeto if missing
            if "pixelSeed" in photons.fields:
                an_pix = ~photons.pixelSeed
            else:
                an_pix = photons.electronVeto > options["e_veto"]
            an_mva = photons.mvaID > options["AN2020_mvaID_min"]

            # High-R9 (>0.85) skips sieie / ECAL iso / track iso cuts per Table 7.
            high_r9 = photons.r9 > options["AN2020_r9_highR9"]

            an_sieie = photons.sieie < options["AN2020_sieie_max_lowR9"]
            an_ecal = photons.ecalPFClusterIso_PUcorr < options["AN2020_ecalPFClusIso_max_lowR9"]
            if "trkSumPtHollowConeDR03" in photons.fields:
                trk_iso = photons.trkSumPtHollowConeDR03
            else:
                trk_iso = photons.trkSumPtSolidConeDR04
            an_trk_abs = trk_iso < options["AN2020_trkIso_max_lowR9"]
            an_trk_rel = (trk_iso / photons.pt) < options["AN2020_trkIso_rel_max"]

            an_shower_iso = ak.where(high_r9, True, an_sieie & an_ecal & an_trk_abs)

            phid_AN2020 = (
                an_r9 & an_eta & an_hoe & an_pix & an_mva & an_trk_rel & an_shower_iso
            )

        photons = ak.with_field(photons, phid_AN2020, "pass_phid_AN2020")
