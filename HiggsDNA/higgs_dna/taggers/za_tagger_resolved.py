...
from higgs_dna.selections import photon_selections
from higgs_dna.selections.photon_selections import to_momentum4d
...
# NOTE: `to_momentum4d` 已移到 higgs_dna/selections/photon_selections.py，
# 此處以 import 重新匯出，維持既有 `za_tagger_resolved.to_momentum4d` 參照不變。
...
class ZaTaggerRun3(Tagger):
    ...
    def select_photons(self, photons, options, electrons, rho, year):
        """Thin wrapper：photon selection 邏輯已抽到
        higgs_dna/selections/photon_selections.py，此處委派、行為與輸出不變。"""
        return photon_selections.select_resolved_photons(
            photons=photons,
            options=options,
            electrons=electrons,
            rho=rho,
    ...
    def select_FSRphotons(self, FSRphotons, electrons, muons, photons, options):
        """Thin wrapper：FSR photon selection 邏輯已抽到
        higgs_dna/selections/photon_selections.py，此處委派、行為與輸出不變。"""
        return photon_selections.select_resolved_fsr_photons(
