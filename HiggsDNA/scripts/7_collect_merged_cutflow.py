...
# MLNANO signal samples (run via za_tagger_merged.py; eos_logs under Sig_MC_MLNANO_all/)
# Low-mA (0.1-0.9 GeV) + transition/intermediate (1-10 GeV)
name_sig_MLNANO_low  = [f"mA_MLNANO_M0p{n}" for n in range(1, 10)]   # M0p1..M0p9
name_sig_MLNANO_mid  = [f"mA_MLNANO_M{n}"   for n in range(1, 11)]   # M1..M10
year_sig_MLNANO = ["2024"]

# MLNANO background samples
name_bkg_MLNANO_DYG  = ["DYGto2LG_MLL50"]
name_bkg_MLNANO_DYJ  = []   # DYJets MLNANO not yet produced
year_bkg_MLNANO = ["2024"]

# MLNANO data samples
name_data_MLNANO = ["EGamma0_Run2024G", "Muon0_Run2024G"]
year_data_MLNANO = ["2024"]
# Name of Bkg Sample
...
# Merged category cutflow types (za_tagger_merged.py)
MERGED_CUTFLOW_TYPES = [
    "zgammas_merged_AN2020",
    "zgammas_merged_AN2020_w",
    "zgammas_merged_ML",
    "zgammas_merged_ML_w",
    "zgammas_merged_ML_ele",
    "zgammas_merged_ML_ele_w",
    "zgammas_merged_ML_mu",
...
LABELS_COMMON = {
    ...
    # merged-specific steps
    "has_1merged": r"$N_{\Gamma}^{\rm AN2020}=1$",
    "has_1MLPhoton_diphoton": r"$N_{\rm MLPhoton}^{\rm dip}>0.5=1$",
...
def iter_all_samples() -> Iterator[Tuple[str, str, str]]:
    ...
    # MLNANO signal (low-mA + mid-range, dataset_type = Sig_MC_MLNANO_all)
    for y in year_sig_MLNANO:
        for ds in name_sig_MLNANO_low + name_sig_MLNANO_mid:
            yield ("Sig_MC_MLNANO_all", ds, y)

    # MLNANO background
    for y in year_bkg_MLNANO:
        for ds in name_bkg_MLNANO_DYG + name_bkg_MLNANO_DYJ:
            yield ("Bkg_MC_MLNANO", ds, y)

    # MLNANO data
    for y in year_data_MLNANO:
        for ds in name_data_MLNANO:
            yield ("Data_MLNANO", ds, y)
