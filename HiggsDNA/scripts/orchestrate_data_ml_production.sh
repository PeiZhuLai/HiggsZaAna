...
for tag in "${TAGS[@]}"; do
    ...
    das=$($HZAPY -c "import json;print(list(json.load(open('$CATALOG'))['$tag']['files'].values())[0][0])")
...
export OUTDIR_BASE=/eos/cms/store/group/phys_susy/pelai/HZa_merged/parquet_friend
bash scripts/run_merged_data_ml_friend.sh >> "$STATUS" 2>&1
log "Step B driver returned. Check per-tag merged_nominal.parquet under $OUTDIR_BASE/Data_20*/"
