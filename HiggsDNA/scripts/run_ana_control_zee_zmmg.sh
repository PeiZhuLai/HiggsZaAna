...
# 生產 Z->ee + Z->mumugamma control sample（驗證 lepton / photon scale factor）。
# tagger : higgs_dna.taggers.control_zee_zmmg_tagger.ControlZeeZmmgTagger
# config : metadata/za_mc_control_zee_zmmg_run3.json
#   -> 套用 PU + HLT + electron(reco/ID) + muon(reco/ID) + photon(ID/CSEV) SF
# 參考 scripts/run_ana_bkgmc_tnp_zmmg.sh
...
outdir="${OUTDIR:-/eos/home-p/pelai/HZa/parquet_control_zee_zmmg_tmp/mc}"
config="${CONFIG:-metadata/za_mc_control_zee_zmmg_run3.json}"
