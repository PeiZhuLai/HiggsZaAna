...
BASE = sys.argv[1] if len(sys.argv) > 1 else \
    "/eos/project/h/htozg-dy-privatemc/pelai/HZa/parquet_friend"
...
def main():
    ...
    for td in tagdirs:
        ...
        # collect the SAME set merge_tag_outputs uses: outputs of successful jobs.
        good_outputs = []
        for jd in jobdirs:
            ...
            if summ and out:
                try:
                    if bool(json.load(open(summ[0])).get("successful")) and os.path.exists(out[0]):
                        good_outputs.append(out[0])
                except Exception:
                    pass
        done = len(good_outputs)
        miss = total - done
