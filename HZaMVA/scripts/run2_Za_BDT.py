...
def compare_train_test(clf,x_train,y_train,z_train,w_train,x_test,y_test,z_test,w_test, bins=50, label=''):
    ...
    plt.savefig(f'./plots_MVA/run2/train_test.pdf', format='pdf')
    plt.show()
...
for hlf, xlabel_hlf, fn in zip(variables+['param']+mass_variables, xlabel, file_name):
    ...
    path = Path("plots_MVA/run2")
    path.mkdir(parents=True, exist_ok=True)
    plt.savefig(f'./plots_MVA/run2/{fn}.pdf', format='pdf')
    plt.show()
...
plt.savefig('./plots_MVA/run2/corr_Sig.pdf', format='pdf')
...
plt.savefig('./plots_MVA/run2/corr_Bkg.pdf', format='pdf')
...
plt.savefig('./plots_MVA/run2/corr_Sig_Bkg.pdf', format='pdf')
...
plt.savefig(f'./plots_MVA/run2/sig_bkg_BDT.pdf', format='pdf')
...
ax.barh(variables_sorted, model.feature_importances_[sorted_idx], color='#276FBF')
...
plt.savefig(f'./plots_MVA/run2/variable_importance.pdf', format='pdf')
plt.show()
...
plt.plot(tpr_train, 1-fpr_train, color='darkorange',
         lw=lw, label=r'Train (AUC = {0:.2f})'.format(auc_train))

plt.plot(tpr_test, 1-fpr_test, color='darkgreen',
         lw=lw, label=r'Test (AUC = {0:.2f})'.format(auc_test))
...
plt.savefig('./plots_MVA/run2/ROC.pdf')
...
def draw_dataMC(var_name, nbins = 50,range_hl=(0,1)):
    ...
    plt.savefig(f'./plots_MVA/run2/weight_check_{var_name}_full_run2.pdf', format='pdf')
    plt.show()
