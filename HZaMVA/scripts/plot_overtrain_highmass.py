#!/usr/bin/env python3
"""Regenerate ONLY the high-mass overtraining plot (train/test BDT score + capped-KS) from the
DEPLOYED model pkl -- NO retraining, so it matches bit-exact the model that produced the limits.
Mirrors the build()/ks/plot logic of train_highmass_final.py, with the CMS-Preliminary + lumi
label of plot_overtrain_lowmass.py so the two overtraining plots share the same format.
CPU (higgs-alp-ana).
"""
import json, pickle
import numpy as np
...
MODEL = f"{_HZAMVA}/using/model_Za_BDT_highmass_run3.pkl"
META  = f"{_HZAMVA}/using/model_Za_BDT_highmass_run3.meta.json"
PLOT  = f"{_HZAMVA}/plots_MVA/run3_highmass/overtrain_highmass_run3.pdf"
HIGHM = [4, 5, 6, 7, 8, 9, 10, 15, 20, 25, 30]
...
def build(tree, n_sig=None, n_bkg=600000):
    sig = {m: _load(f"{ROOT_DIR}/mA_M{m}/run3.root", tree, n_sig, extra=["Z_m", "event"]) for m in HIGHM}
    bk  = _load(f"{ROOT_DIR}/All_Bkg/run3.root", tree, n_bkg, extra=["Z_m", "event"])
    Xs = np.vstack([_feat(sig[m], m)[:, IDX] for m in HIGHM])
    ws = np.concatenate([np.clip(signal_weight(sig[m], m), 0, None) for m in HIGHM])  # signal: factor x sideband reweight
    bhyp = np.random.default_rng(1).choice(HIGHM, size=len(bk["H_m"]))
...
# ---- CMS-Preliminary label, copied verbatim from plot_overtrain_lowmass.py so the two
# ---- overtraining plots (bold "CMS" + italic "Preliminary" + lumi) match exactly. ----
...
add_cms_preliminary(fig, cms_fontsize=18)   # identical CMS-Preliminary style to the low-mass plot
