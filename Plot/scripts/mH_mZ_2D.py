...
def read_hz_arrays(file_path: str):
    """Return (h_mass, z_mass, weight) numpy arrays, or (None, None, None) if not available."""
    ...
    z = None
    for br in Z_MASS_CANDIDATES:
        if br in tree.keys():
            try:
                z = tree[br].array(library="np")
                break
            except Exception:
                continue
    ...
    n = min(len(h), len(z), len(w))
    return h[:n], z[:n], w[:n]

def ensure_outdir_mH_mZ():
    outdir = Path("/afs/cern.ch/work/p/pelai/HZa/HiggsZaAna/Plot/plots/mH_mZ_2D")
...
def draw_hist2d_mH_mZ(
    ...
    """
    PyROOT version: fill TH2D with (z, h) and weight.
    z_edges/h_edges are numpy arrays of bin edges.
    """
    ...
    # 填值
    # 注意：TH2D.Fill(x, y, w) => x=mll (Z), y=mllgg (H)
    for zi, hi, wi in zip(z, h, w):
        if not (np.isfinite(zi) and np.isfinite(hi) and np.isfinite(wi)):
            continue
        h2.Fill(float(zi), float(hi), float(wi))
    ...
    # Title offset（此圖專用微調；想只用全域可刪）
    h2.GetXaxis().SetTitleOffset(1.2)
    h2.GetYaxis().SetTitleOffset(1.1)
    h2.GetZaxis().SetTitleOffset(1.2)

    # LogZ 類似 matplotlib 的 LogNorm
    c = ROOT.TCanvas("c_mH_mZ", "c_mH_mZ", 800, 600)
    c.SetLogz(True)
    ...
    # --- reference lines: mH=95,180 (horizontal) and mZ=50 (vertical), linestyle=2 ---
    ...
    line_mz_50 = ROOT.TLine(50.0, y_min, 50.0, y_max)
    line_mz_50.SetLineStyle(2)
    line_mz_50.SetLineWidth(3)
    line_mz_50.Draw("same")

    # ---斜線：mH + mZ = 185（也就是 y = 185 - x），裁切到畫框範圍內---
    C = 185.0
    pts = []
    ...
    if len(pts) >= 2:
        ...
        if len(uniq) >= 2:
            ...
            line_sum_185 = ROOT.TLine(float(x1), float(y1), float(x2), float(y2))
            line_sum_185.SetLineStyle(9)
            line_sum_185.SetLineWidth(3)
            line_sum_185.SetLineColor(ROOT.kRed + 1)
            line_sum_185.Draw("same")
            ...
            lab.SetTextAngle(-31)  # 旋轉角度（度），可改成 -45, 30... 依需求
            lab.SetTextColor(ROOT.kRed + 1)  # 旋轉角度（度），可改成 -45, 30... 依需求
            lab.DrawLatex(float(tx), float(ty), "m_{ll}+m_{ll#gamma#gamma}=185")
    ...
    # 清理（避免大量 mass points 記憶體累積）
    c.Close()
    ROOT.gDirectory.Delete("h2_mH_mZ;*")
    ...
    for y in bkg_years_all:
        for s in bkg_samples_by_year.get(y, []):
            ...
            if not os.path.exists(fpath):
                missing += 1
                continue
            h, z, w = read_hz_arrays(fpath)
            if h is None:
                continue
            hs.append(h); zs.append(z); ws.append(w)

    if missing > 0:
        print(f"[bkg] missing files skipped: {missing}")
    if len(hs) == 0:
        return np.array([]), np.array([]), np.array([])
    return np.concatenate(hs), np.concatenate(zs), np.concatenate(ws)
...
def collect_all_signal_hzw():
    """Collect (mH, mZ, w) for all signal mass points combined."""
    hs, zs, ws = [], [], []
    missing = 0
...
def main():
    ...
    # 固定 range + step=2
    z_edges = np.arange(40.0, 120.0 + 2.0 + 1e-9, 2.0)
    h_edges = np.arange(70.0, 190.0 + 2.0 + 1e-9, 2.0)

    # background：1 張
    bh, bz, bw = collect_background_hzw()
    if bh.size > 0:
        out_pdf = outdir_hz / "mH_vs_mZ_background.pdf"
        draw_hist2d_mH_mZ(bz, bh, bw, z_edges, h_edges, out_pdf, title=None, label_kind="background")
        print(f"[bkg] saved -> {out_pdf}")
    else:
        print("[bkg] no entries found for mH/mZ.")

    # signal：所有質量點加總：1 張
    ah, az, aw = collect_all_signal_hzw()
    if ah.size > 0:
        out_pdf = outdir_hz / "mH_vs_mZ_signal_allmA.pdf"
        draw_hist2d_mH_mZ(
            az, ah, aw, z_edges, h_edges, out_pdf,
            title=None, label_kind="signal", ma_label="All"
        )
        print(f"[sig:all] saved -> {out_pdf}")
    else:
        print("[sig:all] no entries found for combined mH/mZ.")

    # signal：每個質量點 1 張
    for s in sig_samples:
        ma = mass_from_sig(s)
        sh, sz, sw = collect_signal_hzw_for_sample(s)
        if sh.size == 0:
            continue
        out_pdf = outdir_hz / f"mH_vs_mZ_signal_ma{int(ma)}.pdf"
        draw_hist2d_mH_mZ(
            sz, sh, sw, z_edges, h_edges, out_pdf,
