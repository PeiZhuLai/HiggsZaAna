...
def _plot_year(year: str, points: Dict[int, Dict[str, Dict[str, float]]], out_dir: Path) -> None:
    ...
    leg = ROOT.TLegend(0.15, 0.60, 0.52, 0.90)
    leg.SetBorderSize(0)
    leg.SetFillStyle(0)
    leg.SetTextFont(42)
    ...
    leg_wp = ROOT.TLegend(0.55, 0.70, 0.78, 0.85)  # <- 改這裡調整 3 wps legend 位置
    ...
    if _all_pos_ys:
        ...
        # --- legends: 5 kinds (colors) + 3 wps (black line styles) ---
        leg_log = ROOT.TLegend(0.50, 0.18, 0.90, 0.48)  # keep same pos as linear (change if needed)
        ...
        leg_wp_log = ROOT.TLegend(0.35, 0.28, 0.56, 0.43)  # keep same pos as linear (change if needed)
